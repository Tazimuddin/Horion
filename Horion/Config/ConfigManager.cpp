...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (configExists) {
			...
			currentConfigObj["from"] = "Fakeaway";
		}
		...
		if (Game.getLocalPlayer() != nullptr) {
			...
			if (!helpedUser && name != "default") {
				helpedUser = true;
				Game.getGuiData()->displayClientMessageF("[ %sHorion%s ] %sEnter \"%s%cconfig load default%s\" to load your old config!", DARK_PURPLE, WHITE, YELLOW, WHITE, cmdMgr->prefix, YELLOW);
			}
		}
	} else {
		if (Game.getLocalPlayer() != nullptr) 
